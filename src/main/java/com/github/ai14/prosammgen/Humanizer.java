...
public class Humanizer {
  ...
  private double[] usualWordLength;
  private WordNet wordNet;
  private WordNet informalLanguageWordNet;
  private WritingStyleAnalyzer analyze;
  ...
  public Humanizer(WordNet wordNet, File example) throws IOException, ParseException {
    ...
    this.words = Resources.readLines(Resources.getResource(App.class, "words"), Charsets.UTF_8);
    this.closeCharacters = Resources.readLines(Resources.getResource(App.class, "keyboardclosecharacters"), Charsets.UTF_8);
    this.analyze = new WritingStyleAnalyzer(wordNet);
    ...
    for (String paragraph : allParagraph) {
      ...
      for (int j = 0; j < numberOfWords; ++j) {
        ...
        if ((((j % misspellingRate)) == 0)) { //Check if it should misspell a word according to the calculated probability
          ...
          if (possibleMisspellingWords.isEmpty() && wordsParagraph[j].length() > 0) { //if there is no possible misspelled word (in our file "words")
  ...
  private List<String> checkPossibleSwitcherCharacters(String correctWord) {
    ...
    if (correctWord.length() <= 0) return possibleSwitchingCharacter;
  ...
  /**
   * Given a list of words and a % for each word, choose the word with higher %
   *
